...
interface FinalizeCandidatesOptions {
  ...
  skipRerank: boolean;
  chunkStrategy: import("./chunking.js").ChunkStrategy | undefined;
  primaryQuery: string;
  hooks: SearchHooks | undefined;
}

/**
 * Post-fusion pipeline shared by hybridQuery and structuredSearch: RRF-blend
 * the ranked lists (weighting the top 2 lists 2x — typically the original
 * query's FTS/vec hits), pick each candidate's best-matching chunk, and
 * either return RRF-only results or rerank and blend scores.
 */
async function finalizeCandidates(
  store: Store,
  {
    rankedLists, rankedListMeta, docidMap, candidateLimit, limit, minScore,
    explain, intent, skipRerank, chunkStrategy, primaryQuery, hooks,
...
export async function hybridQuery(
  ...
  const hasVectors = vectorsTableExists(store.db);
  ...
  return finalizeCandidates(store, {
    rankedLists, rankedListMeta, docidMap, candidateLimit, limit, minScore,
    explain, intent, skipRerank, chunkStrategy: options?.chunkStrategy,
    primaryQuery: query, hooks,
  });
}
